...
## v0.10.0 - 캔버스 극대 줌(Zoom) & 클릭 선택 인터랙션 고도화 (완료)

- [x] 최대 줌 배율 한계를 기존 10배에서 **80배**로 대폭 상향 확장
- [x] 줌 수준별 책, 장(Chapter), 개별 구절(절, Verse) 구분 수직 눈금 다이내믹 LOD 렌더러 탑재 (Viewport 필터링 성능 보장)
- [x] PC 마우스 환경 오버 시 상세 카드 노출을 전면 차단하여 시각적 번쩍임 및 피로감 완치
- [x] 오버 시에는 캔버스 마우스 커서를 `pointer` 모양으로 바꾸고 은은한 녹색 가이드선 임시 덧그리기 힌트 제공
- [x] 오직 **직접 클릭(Click) 선택 시에만** 상세 카드 정보 표출 및 핀(Pin) 고정 결합
- [x] 모바일 터치 스와이프 탐색 시에는 스치며 탐색하는 실시간 카드 갱신 구조 분리 보존
